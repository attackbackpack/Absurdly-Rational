...
test("no ?preview= parameter falls back to the default preview", () => {
...
test("a root-relative same-origin path is kept", () => {
  assert.equal(resolvePreviewPath("/preview/index.html"), "/preview/index.html");
  assert.equal(resolvePreviewPath("/index.html"), "/index.html");
  assert.equal(resolvePreviewPath("/preview/"), "/preview/");
});
...
test("a path that is not root-relative falls back to the default", () => {
  assert.equal(resolvePreviewPath("preview/index.html"), DEFAULT_PREVIEW_PATH);
  assert.equal(resolvePreviewPath("../preview/index.html"), DEFAULT_PREVIEW_PATH);
});

test("a path carrying a query or fragment falls back to the default", () => {
  assert.equal(resolvePreviewPath("/preview/?x=1"), DEFAULT_PREVIEW_PATH);
  assert.equal(resolvePreviewPath("/preview/#top"), DEFAULT_PREVIEW_PATH);
});
